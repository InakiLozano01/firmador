...
public class RemoteDocumentSignatureServiceImpl extends AbstractRemoteSignatureServiceImpl
	 ...
	 */
	public RemoteDocumentSignatureServiceImpl(CommonCertificateVerifier certificateVerifier) {
		this.padesService = new PAdESService(certificateVerifier);
	}

	@SuppressWarnings("rawtypes")
	public DocumentSignatureService getPAdESService() {
		return padesService;
	}
	...
	@SuppressWarnings("rawtypes")
	private DocumentSignatureService getServiceForSignature(SignatureForm signatureForm,
			...
			switch (signatureForm) {
				...
				default:
					throw new UnsupportedOperationException(
							"Unrecognized format (XAdES or CAdES are allowed with ASiC) : " + signatureForm);
			}
		} else {
			switch (signatureForm) {
				case XAdES:
	...
	@SuppressWarnings("rawtypes")
	private CounterSignatureService getServiceForCounterSignature(SignatureForm signatureForm,
			ASiCContainerType asicContainerType) {
		if (asicContainerType != null) {
			switch (signatureForm) {
				case XAdES:
			...
			switch (signatureForm) {
				...
				case PAdES:
					throw new UnsupportedOperationException(
							String.format("The Counter Signature is not supported with %s", signatureForm));
				case JAdES:
					return jadesService;
				default:
					throw new UnsupportedOperationException("Unrecognized format " + signatureForm);
	...
	private DocumentSignatureService getServiceForTimestamp(TimestampContainerForm timestampContainerForm) {
		...
		switch (timestampContainerForm) {
			...
			default:
				throw new UnsupportedOperationException(
						"Unrecognized format (only PDF, ASiC-E and ASiC-S are allowed) : " + timestampContainerForm);
		}
	}
	...
	public ToBeSignedDTO getDataToSign(RemoteDocument remoteDocument, RemoteSignatureParameters remoteParameters) {
		...
		// getServiceForSignature(remoteParameters.getSignatureLevel().getSignatureForm(),
		// remoteParameters.getAsicContainerType());
		DSSDocument dssDocument = RemoteDocumentConverter.toDSSDocument(remoteDocument);
		service = (PAdESService) getPAdESService();
		ToBeSigned dataToSign = service.getDataToSign(dssDocument, (PAdESSignatureParameters) parameters);
	...
	@SuppressWarnings({ "rawtypes", "unchecked" })
	@Override
	public RemoteDocument signDocument(RemoteDocument remoteDocument, RemoteSignatureParameters remoteParameters,
		...
		SerializableSignatureParameters parameters = createParameters(remoteParameters);
		DocumentSignatureService service = getServiceForSignature(
	...
	public ToBeSignedDTO getDataToBeCounterSigned(RemoteDocument signatureDocument,
		...
		SerializableCounterSignatureParameters counterSignatureParameters = createCounterSignatureParameters(
				remoteParameters);
		...
		ToBeSigned dataToSign = counterSignatureService.getDataToBeCounterSigned(dssDocument,
	...
	@Override
	public RemoteDocument counterSignSignature(RemoteDocument signatureDocument,
			RemoteSignatureParameters remoteParameters,
			SignatureValueDTO signatureValueDTO) {
		Objects.requireNonNull(signatureDocument, "signatureDocument must be defined!");
		Objects.requireNonNull(remoteParameters, "remoteParameters must be defined!");
