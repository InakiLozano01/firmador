...
public abstract class AbstractRemoteSignatureServiceImpl {
		...
		switch (signatureForm) {
			...
			default:
				throw new UnsupportedOperationException(
						"Unrecognized format (only XAdES or CAdES are allowed with ASiC) : " + signatureForm);
		}
	}

	/**
	 * Creates {@code SerializableSignatureParameters} from
	 * {@code RemoteSignatureParameters}
	 *
	...
	protected void fillParameters(AbstractSignatureParameters<TimestampParameters> parameters,
		 ...
		 * parameters.setMaskGenerationFunction(remoteParameters.
		 ...
		 * parameters.setContentTimestamps(TimestampTokenConverter.toTimestampTokens(
		 * remoteParameters.getContentTimestamps()));
		 * }
		 */
		parameters.setSignatureTimestampParameters(toTimestampParameters(
				remoteParameters.getSignatureTimestampParameters(),
	...
	protected BLevelParameters toBLevelParameters(RemoteBLevelParameters remoteBLevelParameters) {
		...
		if (remoteBLevelParameters.getCommitmentTypeIndications() != null) {
			bLevelParameters.setCommitmentTypeIndications(
					toCommitmentTypeList(remoteBLevelParameters.getCommitmentTypeIndications()));
		}
	...
	protected TimestampParameters toTimestampParameters(RemoteTimestampParameters remoteTimestampParameters) {
		Objects.requireNonNull(remoteTimestampParameters.getTimestampContainerForm(),
				"Timestamp container form is not defined!");
		...
		switch (timestampForm) {
			...
			default:
				throw new UnsupportedOperationException(
						String.format("Unsupported timestamp container form [%s]", timestampForm.getReadable()));
		}
	}

	/**
	 * Transforms ASiC {@code RemoteTimestampParameters} to
	 * {@code TimestampParameters}
	 *
	 * @param remoteTimestampParameters {@link RemoteTimestampParameters}
	 * @param signatureForm             {@link SignatureForm}
	 * @param asicContainerType         {@link ASiCContainerType}
	...
	protected TimestampParameters toTimestampParameters(RemoteTimestampParameters remoteTimestampParameters,
		...
		if (asicContainerType != null) {
			switch (signatureForm) {
				...
				default:
					throw new UnsupportedOperationException(
							String.format("Unsupported signature form [%s] for asic container type [%s]", signatureForm,
									asicContainerType));
			}
		} else {
			switch (signatureForm) {
	...
	/**
	 * Transforms a list of {@code CommitmentTypeEnum}s to a list of
	 * {@code CommitmentType}s
	 *
	...
	private SignatureImageParameters toImageParameters(final RemoteSignatureImageParameters remoteImageParameters) {
		...
		// image
		if (remoteImageParameters.getImage() != null && remoteImageParameters.getImage().getBytes() != null
				&& remoteImageParameters.getImage().getName() != null) {
			imageParameters.setImage(new InMemoryDocument(remoteImageParameters.getImage().getBytes(),
					remoteImageParameters.getImage().getName()));
		}
		// fieldParameters
	...
	private SignatureImageTextParameters toTextParameters(
			final RemoteSignatureImageTextParameters remoteTextParameters) {
		...
		if (remoteTextParameters.getFont() != null && remoteTextParameters.getFont().getBytes() != null) {
			textParameters
					.setFont(new DSSFileFont(new ByteArrayInputStream(remoteTextParameters.getFont().getBytes())));
		}
		// size
	...
	protected SerializableCounterSignatureParameters createCounterSignatureParameters(
		...
		switch (signatureForm) {
			...
			default:
				throw new UnsupportedOperationException(
						"Unsupported signature form for counter signature : " + signatureForm);
		}
