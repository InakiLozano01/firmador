...
def get_data_to_sign_own(pdf, certificates, current_time, field_id, stamp, encoded_image):
    try:
        ...
        response = requests.post('http://192.168.41.190:5555/services/rest/signature/one-document/getDataToSign', json=body)
...
def sign_document_own(pdf, signature_value, certificates, current_time, field_id, stamp, encoded_image):
    try:
        ...
        response = requests.post('http://192.168.41.190:5555/services/rest/signature/one-document/signDocument', json=body)
        if response.status_code == 200:
            if "bytes" in response.json():
                return response.json(), 200
            else:
...
def get_data_to_sign_tapir_jades(json, certificates, current_time, stamp):
    try:
        ...
        response = requests.post('http://locahost:5555/services/rest/signature/one-document/getDataToSign', json=body)
        if response.status_code == 200:
            if "bytes" in response.json():
                return response.json(), 200
            else:
